...
function vel_deficit(tilt, X_D, def_z, C_T, sigy, sigz, Vel_slice)
    ...
    for i = 1:length(Y_values)
        ...
        for j = 1:length(Z_values)
            Y_val = Y_values[i]
            Z_val = Z_values[j]
            Vel_value = vel_deficit_calculations(C_T, tilt, X_D, D, Y_val, Z_val, def_z, sigy, sigz)
...
function vel_deficit_plot(tilt, X_D, def_z, C_T, sigy, sigz, Vel_slice)
    # determine where we are looking at velocity deficit (@ X_D)
    # parameters:
    wakesave = join([string(tilt*180/pi), "_", string(X_D), "_diff_test_constanctct.pdf"])
    wakesave1 = join([string(tilt*180/pi), "_", string(X_D), "_opt_test_constanctct.pdf"])
    wakesave2 = join([string(tilt*180/pi), "_", string(X_D), "_SOWFA_test_constantct.pdf"])
    ...
    Y_values = convert(Array{Float64,1}, Y_values)
    Z_values = convert(Array{Float64,1}, Z_values)
    # print("Y_values: ", Y_values, "\n")
    # find vel_deficit at each of the points
    data1 = heatmap(Vel_deficit_grid, c=:Blues)
    plot(data1, colorbar_title=" \nVelocity Deficit (m/s)", right_margin=5Plots.mm, grid=:false, xlabel="y/D", ylabel="z*", aspect_ratio=:equal)
    savefig(wakesave)
...
function objective_vel_wrapper!(g, x, params)
    ...
    for i = 1:length(tilt_angles)
        ...
        delta_V_model = delta_V_pertilt(tilt_val, slices, CT_val, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03, filename)
        # print("size(delta_V_model): ", delta_V_model, "\n")
        objective = @. abs(delta_V_model)
        objective = sum(objective)/length(slices)
...
function objective_vel_wrapper_plot(x, params)
    ...
    for i = 1:length(tilt_angles)
        ...
        filename = filenames[i]
        # find delta V based on the model for all slices and tilt angles
        delta_V_pertilt_plot(tilt_val, slices, CT_val, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03, filename)
    end
end
...
function one_minus_b(x, slices, CT_val, params)
    ...
    for i = 1:length(tilt)
        for slice in slices
            ...
            b = @. (CT_val*cos(tilt_val))/(8*(sigy*sigz))
            g_val = b-1
            push!(g, g_val)
            push!(sigz_all, sigz)
            push!(sigy_all, sigy)
...
function delta_V_pertilt(tilt, slices, Ct, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03, filename)
    dV = []
    i = 1
    ...
    for slice in slices
        ...
        Vel_val = vel_deficit(tilt, slice, def_z, Ct, sigy, sigz, Vel_slice[:,:,i])
        push!(dV, Vel_val)
        i = i+1
    end
    return dV
...
function delta_V_pertilt_plot(tilt, slices, Ct, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03, filename)
    i = 1
    Vel_slice = load(filename, "Vel_slices")
    for slice in slices
        def_z = 90.0.+deflection_est(tilt, slice)*126.0
        sigy, sigz = sigysigz(slice, tilt, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03)
        vel_deficit_plot(tilt, slice, def_z, Ct, sigy, sigz, Vel_slice[:,:,i])
        i = i+1
    end
end
...
function run_optimization_SNOPT()
    ...
    sigz02 = -0.014
    sigz03 = 0.0419
    x0 = [kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03]
    ...
    snopt_options = Dict(
        "Derivative option" => 1,
        "Verify level" => 0,
        "Major optimality tolerance" => 8e-4,
    ...
    ng = 90
    lx = [-Inf,-Inf,-Inf,-Inf,-Inf,-Inf,-Inf, -Inf,0.01]
    ux = [Inf,Inf,Inf,Inf,Inf,Inf,Inf, Inf,Inf]
    # lg=[-Inf*ones(ng)]
    # ug=[zeros(ng)]
    lg=[-Inf*ones(30); 0.35*ones(60)]
    ug=[0*ones(30); 0.7*ones(60)]
    ...
    filename_opt = "optimal_solution_newc_testing_constantct.jld"
    fopt_optimal = 100000
    best = x0
    for i = 1:iterations
        print("iterations: ", i, "\n")
