...
function SOWFA_analysis(slices, movname, gauss_namez, gauss_namey, tol_comp, file_1, smoothing, tilt)
    ...
    for j in 1:length(smoothing)
        ...
        for i in 1:length(slices)
            ...
            DNNname = join([movname, "_", string(slices[i]), "_DNN", ".pdf"])
            ...
            data2 = heatmap((crop_Y_plot_fine.-2500)/126, ((crop_Z_plot_fine.-90.0)/90.0), cropped_SOWFA_data_comp_neg_fine, c = :Blues, clim=(0,maximum(matrix)))
            plot(data2,xlabel="x/D",colorbar_title=" \nVelocity Deficit (m/s)", xlim=(-2, 2),ylim=(0, 3),  c = :Blues, yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz,foreground_color_legend = nothing)
            # plot!(size=(1600, 400))
            plot!(annotations = ([-2.2], 1.6,text("z*", fontylabel)), left_margin=7Plots.mm, right_margin=10Plots.mm)
            savefig(DNNname)
        end
    end
...
function SOWFA_analysis_save(slice, movname, gauss_namez, gauss_namey, tol_comp, file_1, smoothing, tilt)
    ...
    DNNname = join([movname, "_", str_slice, "_DNN", ".pdf"])
    ...
    # scale the matrix to [0, 255]
    matrix = cropped_SOWFA_data_comp_neg
    # image_scaled = round.(Int, (255/8.1)*(matrix))
    ...
    # crop_Y_plot_fine =  range(2245.0, stop=2745.0, length=51)
    # crop_Z_plot_fine = range(5.0, stop=304.99999999999994, length=31)
    AAA = 2245.0:((2745.0-2245.0)/51.0):2745.0
    crop_Y_plot_fine = AAA[2:end]
    crop_Z_plot_fine = 5.0:((304.999999999994-5.0)/31.0):304.99999999994

    crop_Y_plot_fine = convert(Vector{Float64}, crop_Y_plot_fine)
    crop_Z_plot_fine = convert(Vector{Float64}, crop_Z_plot_fine)

    print("size cropY: ", size(crop_Y_plot_fine), "\n")
    print("size cropZ: ", size(crop_Z_plot_fine), "\n")
    print("cropped_SOWFA_data_comp_neg_fine: ", size(cropped_SOWFA_data_comp_neg), "\n")
    data2 = heatmap((crop_Y_plot_fine.-2500)/126, ((crop_Z_plot_fine.-90.0)/90.0), cropped_SOWFA_data_comp_neg, c = :Blues, clim=(0,maximum(matrix)))
    plot(data2,xlabel="x/D",colorbar_title=" \nVelocity Deficit (m/s)", xlim=(-2, 2),ylim=(-1, 2.3),aspect_ratio=0.7,  c = :Blues, yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz,foreground_color_legend = nothing)
    # plot!(size(1600, 200))
    f(t) = 0.0 .+ (1/2).*cos.(t)
    g(t) = 0.0 .+ (126.0/90.0)*(1/2).*sin.(t)
    range = 0:0.01:2*pi
    x_values = f(range)
    y_value = g(range)
    plot!(x_values, y_value, color=:black, label="Rotor")

    plot!(annotations = ([-2.4], 0.7,text("z*", fontylabel)), left_margin=7Plots.mm, right_margin=10Plots.mm)

    # wake center for 12.5 degree tilt
    # plot!([0.021818561436475647], [0.5379997912099381 - 1.0], seriestype=:scatter, color="red", label="Wake Center")
    # savefig("12.5_crossstream.pdf")

    # wake center for -15 degree tilt
    plot!([-0.23664122137404586], [1.5204081632653064 - 1.0], seriestype=:scatter, color="red", legend=:false)
    savefig("neg15_crossstream.pdf")
    # savefig(DNNname)
    # savefig("crosstream_25.pdf")
...
file_9 = "/Users/jamescutler/Downloads/byu_tilt_runs_single_opp/c_003_sp7_1turb_hNormal_D126_tilt_25/lite_data_003.ftr"
