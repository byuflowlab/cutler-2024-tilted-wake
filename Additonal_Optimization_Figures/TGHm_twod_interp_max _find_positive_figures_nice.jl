...
# Scatter plot for σ_y0
ax.scatter(TILT*(180/pi), eps_y, color="dodgerblue")

# Line plot for the tilt_plot data
ax.plot(tilt_plot*(180/pi), (tilt_plot ./ tilt_plot) * eps_y_d, color="dodgerblue")

# Scatter plot for σ_{z0}
ax.scatter(TILT*(180/pi), eps_z_up, color="black")

# Line plot for Y_values_up
ax.plot(tilt_plot*(180/pi), Y_values_up, color="black")

# Set labels, font sizes, and ticks
ax.set_xlabel("Tilt (degrees)", fontsize=fontsz, fontname=fontname)
ax.set_xticks([2.5, 5.0, 7.5, 10.0, 12.5])
...
# Add annotations with LaTeX formatting and Times New Roman font
ax.text(7.5, 0.21, "\$\\sigma_{z0} = $sigz_up_a - $sigz_up_plot \\ln (\\gamma *)\$", fontsize=fontsz, fontname=fontname)
ax.text(8.0, 0.25, "\$\\sigma_{y0} = $eps_y_d\$", fontsize=fontsz, fontname=fontname, color="dodgerblue")
ax.text(0.2, 0.228, "\$\\sigma_0\$", fontsize=fontsz, fontname=fontname)

# Adjust layout
fig.tight_layout()

# Save the figure
fig.savefig("sigma_0_degree.pdf", bbox_inches="tight")
...
# Scatter plot for ky
ax.scatter(Tilt_degree, ky_arr, color="dodgerblue")

# Scatter plot for kz
ax.scatter(Tilt_degree, kz_arr, color="black")

# Line plot for Z_values
ax.plot(tilt_plot * (180 / pi), Z_values, color="dodgerblue")

# Line plot for Y_values
ax.plot(tilt_plot * (180 / pi), Y_values, color="black")
...
# Add annotations with LaTeX formatting and Times New Roman font
ax.text(7.0, 0.0325, "\$kz = $kz1_val γ^2 + $kz_val γ + $eps_z \$", fontsize=fontsz, fontname=fontname, color="black")
ax.text(6.0, 0.022, "\$ky = $ky_val γ + $eps_y \$", fontsize=fontsz, fontname=fontname, color="dodgerblue")
ax.text(-1.25, 0.0265, "\$\\text{k-value} \$", fontsize=fontsz, fontname=fontname)
...
# Save the figure
fig.savefig("kykztilt_both_new.pdf", bbox_inches="tight")
