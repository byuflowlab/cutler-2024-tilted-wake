...
ax.plot(slices, RMS1, color="lightblue")
ax.plot(slices, RMS2, color="lightskyblue")
ax.plot(slices, RMS3, color="deepskyblue")
ax.plot(slices, RMS4, color="dodgerblue")
ax.plot(slices, RMS5, color="steelblue")

ax.set_xlabel("x/D (Downstream Distance)", fontsize=fontsz)
...
ax.text(X_ax, RMS1[end]-0.5, "\$2.5°\$", color="lightblue", fontsize=fontsz)
ax.text(X_ax, RMS2[end]-0.8, "\$5.0°\$", color="lightskyblue", fontsize=fontsz)
ax.text(X_ax, RMS3[end]-1.0, "\$7.5°\$", color="deepskyblue", fontsize=fontsz)
ax.text(X_ax, RMS4[end]-1.0, "\$10.0°\$", color="dodgerblue", fontsize=fontsz)
ax.text(X_ax, RMS5[end]-1.0, "\$12.5°\$", color="steelblue", fontsize=fontsz)
ax.text(5.1, -7.5, "\$\\Delta \\text{RMS %}\$", fontsize=fontsz)

fig.tight_layout()

fig.savefig("diff_objective_pow.pdf", bbox_inches="tight")
...
ax.plot(tilt*(180/pi), kz_pred, color="dodgerblue")
ax.plot(tilt*(180/pi), kz_opt, color="black")

ax.set_xlabel("Tilt (degrees)", fontsize=fontsz)
...
ky1_c = 0.048
ky2_c = 0.018
ax.plot(tilt*(180/pi), ky_pred, color="dodgerblue")
ax.plot(tilt*(180/pi), ky_opt, color="black")
ax.text(3.25, 0.0285, "\$kz = $kz1_p\\gamma^{2} + $kz2_p\\gamma + $kz3_p \$", color="black", fontsize=fontsz)
ax.text([2.5], 0.0238, "\$ ky = $ky1_p\\gamma + $ky2_p \$", color="black", fontsize=fontsz)
ax.text([7.1], 0.0325, "\$ kz = $kz1_c\\gamma^{2} + $kz2_c\\gamma + $kz3_c\$", color="dodgerblue", fontsize=fontsz)
ax.text([6.0], 0.022, "\$ ky = $ky1_c\\gamma + $ky2_c\$", color="dodgerblue", fontsize=fontsz)
ax.text([-1.1], 0.0259, "\$ \\text{k-value} \$", fontsize=fontsz)

# Adjust layout
fig.tight_layout()

# Save the figure
fig.savefig("kykz_both_opt_pow.pdf", bbox_inches="tight")
...
ax.plot(tilt*(180/pi), sigz_pred, color="dodgerblue")
ax.plot(tilt*(180/pi), sigz0_opt, color="black")
ax.set_xlabel("Tilt (degrees)", fontsize=fontsz)
ax.set_xticks([2.5, 5.0, 7.5, 10.0, 12.5])
ax.tick_params(axis="x", labelsize=fontsz)
ax.tick_params(axis="y", labelsize=fontsz)
...
ax.text([2.5], 0.196, "\$ \\sigma_{z0} = 0.168 - 0.014\\ ln (\\gamma - 0.0419)\$", color="dodgerblue", fontsize=fontsz)
ax.text([5.5], 0.22,"\$ \\sigma_{z0} = $sigz01_p $sigz02_p\\ ln (\\gamma + $sigz03_p)\$", color="black", fontsize=fontsz)
ax.text([0.5], 0.225,"\$ \\sigma_0\$", color="black", fontsize=fontsz)
# Adjust layout
fig.tight_layout()

# Save the figure
fig.savefig("sigz0_opt_pow.pdf", bbox_inches="tight")
