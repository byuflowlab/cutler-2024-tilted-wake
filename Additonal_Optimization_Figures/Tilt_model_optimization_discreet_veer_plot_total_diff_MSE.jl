...
function SOWFA_analysis(slices, movname, gauss_namez, gauss_namey, tol_comp, file_1, smoothing, tilt)
    ...
    for j in 1:length(smoothing)
        ...
        for i in 1:length(slices)
            ...
            plot(data1, colorbar_title=" \nVelocity Deficit (m/s)",clim=(0,1.2), right_margin=5Plots.mm, grid=:false, xlabel="y/D")
            ...
            plot!(annotations = ([-2.7], 0.75,text("z*", fontylabel)))
            ...
            plot!(x_values, y_values, label="Rotor",linewidth=linew+1,legend=:false, color="firebrick", xlabel="y/D", xrotation = 0)
...
function SOWFA_analysis_save(slice, movname, gauss_namez, gauss_namey, tol_comp, file_1, smoothing, tilt)
    ...
    # # scatter!(([Y_cm_comp_threshold].-2500)/126, [Z_cm_comp_threshold]/90, color="brown", label="Center based on max Deficit")
    # # scatter!(([Y_cm_comp_max].-2500)/126, [Z_cm_comp_max]/90, color="magenta", label="Center based on max Deficit")
    scatter!(([Y_cm_comp].-2500)/126, [Z_cm_comp-90.0]/90,markerstrokewidth=0, markersize = marks, color="green", label="Wake Center")
    plot!(x_values, y_values, label="Rotor",linewidth=linew+1,legend=:false, color="firebrick", xlabel="y/D", xrotation = 0)
    fontylabel = text("", 15).font
    plot!(annotations = ([-2.7], 0.75,text("z*", fontylabel)))
    xlims!((-2,2))
...
function vel_deficit(tilt, X_D, def_z, C_T, sigy, sigz, Vel_slice, alpha_in)
    ...
    for i = 1:length(Y_values)
        ...
        for j = 2:length(Z_values)
            Y_val = Y_values[i]
            Z_val = Z_values[j]
            Vel_value = vel_deficit_calculations(C_T, tilt, X_D, D, Y_val, Z_val, def_z, sigy, sigz, alpha_in)
            Vel_slice_val = Vel_slice[j, i]
            diff = abs((Vel_value*8.1)-Vel_slice_val)
...
function vel_deficit_plot_diff(tilt, X_D, def_z, C_T, sigy, sigz, Vel_slice, alpha_in, alpha_in_0, sigz0, sigy0)
    # determine where we are looking at velocity deficit (@ X_D)
    # parameters:
    ...
    for i = 1:length(Y_values)
        ...
        for j = 2:length(Z_values)
            ...
            Z_val = Z_values[j]
            Vel_value = vel_deficit_calculations(C_T, tilt, X_D, D, Y_val, Z_val, def_z, sigy, sigz, alpha_in)
            Vel_value_0 = vel_deficit_calculations(C_T, tilt, X_D, D, Y_val, Z_val, def_z, sigy0, sigz0, alpha_in_0)
            Vel_slice_val = Vel_slice[j,i]
            diff = ((Vel_value*8.1)-Vel_slice_val)^2
            diff_0 = ((Vel_value_0*8.1)-Vel_slice_val)^2
    ...
    # find vel_deficit at each of the points
    # data1 = heatmap(Y_values, Z_values[2:end], Vel_0_grid, c=:Blues, clim=(0.0, 2.5))
    # plot(data1, colorbar_title=" \nVelocity Deficit (m/s)", xlimit = [-2.0, 2.0], right_margin=5Plots.mm, grid=:false, xlabel="y/D", ylabel="z*",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
    # savefig(wakesave5)

    # data1 = heatmap(Y_values, Z_values[2:end], Vel_opt_grid, c=:Blues, clim=(0.0, 2.5))
    # plot(data1, colorbar_title=" \nVelocity Deficit (m/s)", xlimit = [-2.0, 2.0], right_margin=5Plots.mm, grid=:false, xlabel="y/D", ylabel="z*",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
    # savefig(wakesave4)

    # data1 = heatmap(Y_values, Z_values[2:end], Vel_SOWFA_grid, c=:Blues, clim=(0.0, 2.5))
    # plot(data1, colorbar_title=" \nVelocity Deficit (m/s)", xlimit = [-2.0, 2.0], right_margin=5Plots.mm, grid=:false, xlabel="y/D", ylabel="z*",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
    # savefig(wakesave3)

    # data1 = heatmap(Y_values, Z_values[2:end], Vel_diff_0_grid, c=:Blues, clim=(0.0,1.0))
    # plot(data1, xlimit = [-2.0, 2.0], colorbar_title="", right_margin=15Plots.mm, grid=:false, xlabel="y/D", ylabel="z*",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
    # plot!(annotations = ([3.5], 0.75,text(L"\left|\Delta U_{org}\right|", fontylabel)))
    # savefig(wakesave2)

    # data1 = heatmap(Y_values, Z_values[2:end], Vel_diff_opt_grid, c=:Blues, clim=(0.0, 1.0))
    # plot(data1, xlimit = [-2.0, 2.0], colorbar_title="", right_margin=15Plots.mm, grid=:false, xlabel="y/D", ylabel="z*",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
    # plot!(annotations = ([3.5], 0.75,text(L"|\Delta U_{opt}|", fontylabel)))
    # savefig(wakesave1)

    # RdBu
    # data1 = heatmap(Y_values, Z_values[2:end], Vel_total_diff, c=:RdBu, clim=(-.2, .2))
    # plot(data1, colorbar_title=" \\n \\n |\\Delta V_{orig}| - |\\Delta V_{opt}|", xlimit = [-2.0, 2.0], right_margin=15Plots.mm, grid=:false, xlabel="y/D", ylabel="z*",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
    # savefig(wakesave)
    
    # data1 = heatmap(Y_values, Z_values[2:end], Vel_total_diff, c=:RdBu, clim=(-.2, .2))
    # plot(data1, xlimit = [-2.0, 2.0], right_margin=15Plots.mm, grid=:false, xlabel="y/D", ylabel="z*",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
    # plot!(annotations = ([3.1], 0.75,text(L"\left|\Delta U_{org}\right| - |\Delta U_{opt}|", fontylabel)))
    # savefig(wakesave)
    ...
    return Vel_diff_0_grid, Vel_diff_opt_grid, Vel_total_diff
end

function vel_deficit_plot(tilt, X_D, def_z, C_T, sigy, sigz, Vel_slice, alpha_in)
    # determine where we are looking at velocity deficit (@ X_D)
    # parameters:
    # wakesave = join([string(tilt*180/pi), "_", string(X_D), "_diff_test.pdf"])
    # wakesave1 = join([string(tilt*180/pi), "_", string(X_D), "_opt_test.pdf"])
    ...
    plot(data1, colorbar_title=" \nVelocity Deficit (m/s)", right_margin=5Plots.mm, grid=:false, xlabel="y/D")
    fontylabel = text("", 15).font
    plot!(annotations = ([-2.7], 0.75,text("z*", fontylabel)))
    savefig(wakesave)
...
function vel_deficit_calculations(C_T, tilt, X_D, D, y_val, z_val, deflection, sigy_D, sigz_D, alpha_in)
    # sigy_D, sigz_D = sigysigz(X_D, tilt)
    sigy = sigy_D*D
    sigz = sigz_D*D
    # print("sigy: ", sigy, "\n")
...
function objective_vel_wrapper_plot_difference_every_tilt(x, x0_opt, params)
    # parameters
    slices = 7.0:1.0:12.0
    tilt_angles = params.tilt
    ...
    for i = 1:length(tilt_angles)
        ...
        if i == 1
            # find delta V based on the model for all slices and tilt angles
            vel0, velopt = delta_V_pertilt_plot_diff_every_tilt(tilt_val, slices, CT_val[i], kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03, filename, alpha_in, alpha_in_0,
            ...
            vel0, velopt = delta_V_pertilt_plot_diff_every_tilt(tilt_val, slices, CT_val[i], kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03, filename, alpha_in, alpha_in_0,
                                        kz1_0, kz2_0, kz3_0, ky1_0, ky2_0, sigy01_0, sigz01_0, sigz02_0, sigz03_0)
            vel0_total = @. vel0_total + vel0
            velopt_total = @. velopt_total + velopt
        end
        ...
        vel_diff_total = @. vel0_total - velopt_total

        wakesave = join(["Total_diff_0_MSE.pdf"])
        wakesave1 = join(["Total_diff_opt_MSE.pdf"])
        wakesave2 = join(["Total_diff_0_minus_opt_MSE.pdf"])

        default(titlefont= ("times"), guidefont=("times"), tickfont=("times"))
        fontylabel = text("", 15).font
        # data1 = heatmap(Y_values, Z_values[2:end], vel0_total, c=:Blues)
        data1 = heatmap(Y_values, Z_values[2:end], vel0_total, c=:Blues, clim=(0.0, 10.3))
        plot(data1, colorbar_title="", xlimit = [-2.0, 2.0], right_margin=20Plots.mm,left_margin=20Plots.mm, grid=:false, xlabel="y/D",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
        plot!(annotations = ([3.5], 0.75,text(L"\left|\Delta U\right|^2", fontylabel)), aspect_ratio=:equal)
        plot!(annotations = ([-2.7], 0.75,text("z*", fontylabel)), ylimit=[-0.918, 2.4])
        savefig(wakesave)

        # data1 = heatmap(Y_values, Z_values[2:end], velopt_total, c=:Blues)
        data1 = heatmap(Y_values, Z_values[2:end], velopt_total, c=:Blues, clim=(0.0, 10.3))
        plot(data1, colorbar_title="", xlimit = [-2.0, 2.0],  right_margin=20Plots.mm,left_margin=20Plots.mm, grid=:false, xlabel="y/D",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
        plot!(annotations = ([3.5], 0.75,text(L"\left|\Delta U\right|^2", fontylabel)), aspect_ratio=:equal)
        plot!(annotations = ([-2.7], 0.75,text("z*", fontylabel)), ylimit=[-0.918, 2.4])
        savefig(wakesave1)

        data1 = heatmap(Y_values, Z_values[2:end], vel_diff_total, c=:Blues)
        plot(data1, xlimit = [-2.0, 2.0],  right_margin=20Plots.mm,left_margin=20Plots.mm, grid=:false, xlabel="y/D",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
        plot!(annotations = ([3.5], 0.75,text(L"\left|\Delta U_{0}\right|^2 - |\Delta U_{opt}|^2", fontylabel)), aspect_ratio=:equal)
        plot!(annotations = ([-2.7], 0.75,text("z*", fontylabel)), ylimit=[-0.918, 2.4])
        savefig(wakesave2)
    ...
    for i = 1:length(slices)
        slice = slices[i]
        delta_V_pertilt_plot_diff_every_slice(i, tilt_angles, slice, CT_val, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03, filenames, alpha_in, alpha_in_0,
                                    kz1_0, kz2_0, kz3_0, ky1_0, ky2_0, sigy01_0, sigz01_0, sigz02_0, sigz03_0)
    end
...
function delta_V_pertilt(tilt, slices, Ct, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03, filename, alpha_in)
    ...
    for slice in slices
        ...
        Vel_val = vel_deficit(tilt, slice, def_z, Ct, sigy, sigz, Vel_slice[:,:,i], alpha_in)
...
function delta_V_pertilt_plot(tilt, slices, Ct, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03, filename, alpha_in)
    ...
    for slice in slices
        def_z = 90.0.+deflection_est(tilt, slice)*126.0
        sigy, sigz = sigysigz(slice, tilt, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03)
        vel_deficit_plot(tilt, slice, def_z, Ct, sigy, sigz, Vel_slice[:,:,i], alpha_in)
        i = i+1
    end
end
...
function delta_V_pertilt_plot_diff(tilt, slices, Ct, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03, filename, alpha_in, alpha_in_0,
    ...
    for slice in slices
        ...
        vel_deficit_plot_diff(tilt, slice, def_z, Ct, sigy, sigz, Vel_slice[:,:,i], alpha_in, alpha_in_0, sigz0, sigy0)
        i = i+1
    end
end

function delta_V_pertilt_plot_diff_every_tilt(tilt, slices, Ct, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03, filename, alpha_in, alpha_in_0,
                                            kz1_0, kz2_0, kz3_0, ky1_0, ky2_0, sigy01_0, sigz01_0, sigz02_0, sigz03_0)
    i = 1
    Vel_slice = load(filename, "Vel_slices")
    vel0 = []
    ...
    for slice in slices
        if i == 1
            ...
            sigy, sigz = sigysigz(slice, tilt, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03)
            sigy0, sigz0 = sigysigz(slice, tilt, kz1_0, kz2_0, kz3_0, ky1_0, ky2_0, sigy01_0, sigz01_0, sigz02_0, sigz03_0)
            Vel_diff_0, Vel_diff_opt, Vel_total_diff = vel_deficit_plot_diff(tilt, slice, def_z, Ct, sigy, sigz, Vel_slice[:,:,i], alpha_in, alpha_in_0, sigz0, sigy0)
            i = i+1
            vel0 = Vel_diff_0
            velopt = Vel_diff_opt
            ...
            sigy0, sigz0 = sigysigz(slice, tilt, kz1_0, kz2_0, kz3_0, ky1_0, ky2_0, sigy01_0, sigz01_0, sigz02_0, sigz03_0)
            Vel_diff_0, Vel_diff_opt, Vel_total_diff = vel_deficit_plot_diff(tilt, slice, def_z, Ct, sigy, sigz, Vel_slice[:,:,i], alpha_in, alpha_in_0, sigz0, sigy0)
            vel0 = @. vel0 + Vel_diff_0
            velopt = @. velopt + Vel_diff_opt
            
            i = i+1
        end
    end
    vel_total_diff = @. vel0 - velopt
    y_array = 2245.0:(2745.0-2245.0)/50:2745.0
    z_array = 5.0:(304.99999999994 - 5.0)/30:304.999999999994
    Y_values = y_array .- 2500.0
    Z_values = z_array .- 90.0
    ...
    wakesave = join([string(tilt*180/pi), "_0_MSE.pdf"])
    wakesave1 = join([string(tilt*180/pi), "_opt_MSE.pdf"])
    wakesave2 = join([string(tilt*180/pi), "_total_MSE.pdf"])

    fontylabel = text("", 15).font
    default(titlefont= ("times"), guidefont=("times"), tickfont=("times"))
    # find vel_deficit at each of the points
    # data1 = heatmap(Y_values, Z_values[2:end], vel0, c=:Blues)
    data1 = heatmap(Y_values, Z_values[2:end], vel0, c=:Blues, clim=(0.0, 2.5))
    plot(data1, colorbar_title="", xlimit = [-2.0, 2.0], right_margin=20Plots.mm,left_margin=20Plots.mm, grid=:false, xlabel="y/D",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
    plot!(annotations = ([3.5], 0.75,text(L"\left|\Delta U\right|^2", fontylabel)), aspect_ratio=:equal)
    plot!(annotations = ([-2.7], 0.75,text("z*", fontylabel)), ylimit=[-0.918, 2.4])
    savefig(wakesave)

    # data1 = heatmap(Y_values, Z_values[2:end], velopt, c=:Blues)
    data1 = heatmap(Y_values, Z_values[2:end], velopt, c=:Blues, clim=(0.0, 2.5))
    plot(data1, colorbar_title="", xlimit = [-2.0, 2.0], right_margin=20Plots.mm,left_margin=20Plots.mm, grid=:false, xlabel="y/D",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
    plot!(annotations = ([3.5], 0.75,text(L"\left|\Delta U\right|^2", fontylabel)), aspect_ratio=:equal)
    plot!(annotations = ([-2.7], 0.75,text("z*", fontylabel)), ylimit=[-0.918, 2.4])
    savefig(wakesave1)

    # data1 = heatmap(Y_values, Z_values[2:end], vel_total_diff, c=:Blues)
    data1 = heatmap(Y_values, Z_values[2:end], vel_total_diff, c=:Blues, clim=(0.0, 1.0))
    plot(data1, xlimit = [-2.0, 2.0], right_margin=20Plots.mm,left_margin=20Plots.mm, grid=:false, xlabel="y/D",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
    plot!(annotations = ([3.1], 0.75,text(L"\left|\Delta U_{0}\right|^2 - |\Delta U_{opt}|^2", fontylabel)), aspect_ratio=:equal)
    plot!(annotations = ([-2.7], 0.75,text("z*", fontylabel)), ylimit=[-0.918, 2.4])
    savefig(wakesave2)
...
function delta_V_pertilt_plot_diff_every_slice(j, tilt_angles, slice, Cts, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03, filenames, alpha_in, alpha_in_0,
                                            kz1_0, kz2_0, kz3_0, ky1_0, ky2_0, sigy01_0, sigz01_0, sigz02_0, sigz03_0)
    i = 1
    # Vel_slice = load(filename, "Vel_slices")
    vel0 = []
    ...
    for tilt in tilt_angles
        ...
        if i == 1
            ...
            sigy, sigz = sigysigz(slice, tilt, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03)
            sigy0, sigz0 = sigysigz(slice, tilt, kz1_0, kz2_0, kz3_0, ky1_0, ky2_0, sigy01_0, sigz01_0, sigz02_0, sigz03_0)
            Vel_diff_0, Vel_diff_opt, Vel_total_diff = vel_deficit_plot_diff(tilt, slice, def_z, Ct, sigy, sigz, Vel_slice[:,:,j], alpha_in, alpha_in_0, sigz0, sigy0)
            i = i+1
            vel0 = Vel_diff_0
            velopt = Vel_diff_opt
    ...
    wakesave = join([string(slice), "_0_MSE.pdf"])
    wakesave1 = join([string(slice), "_opt_MSE.pdf"])
    wakesave2 = join([string(slice), "_total_MSE.pdf"])
...
function check_with_original_over_tilt(x0_opt)
    # compare to original
    file_0 = "/Users/jamescutler/Downloads/byu_tilt_runs_single/c_000_sp7_1turb_hNormal_D126_tilt_base/lite_data_000.ftr"
    file_1 = "/Users/jamescutler/Downloads/byu_tilt_runs_single_opp/c_001_sp7_1turb_hNormal_D126_tilt_2.5/lite_data_001.ftr"
    file_2 = "/Users/jamescutler/Downloads/byu_tilt_runs_single_opp/c_004_sp7_1turb_hNormal_D126_tilt_5/lite_data_004.ftr"
    ...
    objective_vel_wrapper_plot_difference_every_tilt(x0, x0_opt, params)
...
check_with_original_over_tilt(x)
...
# # plot!(slices, fopt_detailed[1], seriestype=:scatter, color=:lightskyblue2)
# plot!(slices, fopt_detailed_original[2], color=:skyblue2, label="5.0")
# # plot!(slices, fopt_detailed[2], seriestype=:scatter, color=:skyblue2, label="5.0")
# plot!(slices, fopt_detailed_original[3], color=:steelblue2, label="7.5")
# # plot!(slices, fopt_detailed[3], seriestype=:scatter, color=:steelblue2, label="7.5")
# plot!(slices, fopt_detailed_original[4], color=:dodgerblue, label="10.0", xlabel="x/D (Downstream Distance)")
# # plot!(slices, fopt_detailed[4], seriestype=:scatter, color=:dodgerblue)
...
# # plot!(slices, fopt_detailed[5], seriestype=:scatter, color=:dodgerblue3)
...
# plot!(annotations = ([5.9], 0.142,text("RMS", fontylabel)), ylims=[0.098, 0.187])
# savefig("optimized_objective.pdf")
...
# """Plot diff"""
# plot(slices, f_diff[1], color=:lightskyblue2, label="2.5")
# plot!(slices, f_diff[2], color=:skyblue2, label="5.0")
# plot!(slices, f_diff[3], color=:steelblue2, label="7.5")
# plot!(slices, f_diff[4], color=:dodgerblue, label="10.0", xlabel="x/D (Downstream Distance)")
# plot!(slices, f_diff[5], color=:dodgerblue3, label="12.5",legend=:false,xlimit = [7, 12.5], legendfontsize=10, grid=false, xticks=[7, 8, 9, 10, 11, 12], dpi=300)
# plot!(annotations = ([X_ax], f_diff[1][end],text("2.5°", font25_c)))
# plot!(annotations = ([X_ax], f_diff[2][end]+0.0005, text("5.0°", font5_c)))
# plot!(annotations = ([X_ax], f_diff[3][end], text("7.5°", font75_c)))
# plot!(annotations = ([X_ax], f_diff[4][end]-0.0005, text("10.0°", font10_c)))
# plot!(annotations = ([X_ax], f_diff[5][end], text("12.5°", font125_c)), left_margin=15Plots.mm, transparent=true, tickfontsize=12, guidefontsize=12)
# plot!(annotations = ([5.76], 0.011,text("\\Delta \\ RMS", fontylabel)))
# savefig("diff_objective.pdf")
...
# plot!(annotations = ([5.9], 0.142,text("RMS", fontylabel)), ylims=[0.098, 0.187])
# savefig("original_objective.pdf")
...
# plot!(annotations = ([-.1], 0.0255,text("k-value", fontylabel)))
# savefig("kykz_both_opt.pdf")
# # check sigy0
...
# plot!(annotations = ([X_ax1-0.05], sigz_2_5_fine[3]+Z_fix+0.020-(sigz_2_5_fine[3]-sigy_2_5_fine[1])/2, text("$kz2_5", font25)))
...
# plot!(annotations = ([X_ax1], sigz_7_5_fine[3]+Z_fix-0.001-(sigz_7_5_fine[3]-sigz_7_5_fine[1])/2, text("$kz7_5", font75)))
# plot!(Xline, Z_7_5, color=:steelblue2)
# plot!(annotations = ([X_ax1], sigz_10_fine[3]+Z_fix-0.001-(sigz_10_fine[3]-sigz_10_fine[1])/2, text("$kz_10", font10)))
# plot!(Xline, Z_10, color=:dodgerblue)
# plot!(annotations = ([X_ax1], sigz_12_5_fine[3]+Z_fix-0.002-(sigz_12_5_fine[3]-sigz_12_5_fine[1])/2, text("$kz12_5", font125)))
# plot!(Xline, Z_12_5, color=:dodgerblue3)
# # plot!(annotations = ([X_ax2], [Y_ax1], "1", 9))
# plot!(annotations = ([Y_ax], 0.49,text("{\\sigma_z/D}", fontylabel)))
# plot!(annotations = ([X_ax], sigz_12_5[end],text("12.5°", font125_c)))
...
# plot!(annotations = ([X_ax1], sigy_12_5_fine[3]+Z_fix+0.002-(sigy_12_5_fine[3]-sigy_12_5_fine[1])/2, text("$ky12_5", font125)))
# plot!(Xline, Y_12_5, color=:dodgerblue3)
# plot!(annotations = ([Y_ax], 0.495,text("{\\sigma_y/D}", fontylabel)))
# plot!(annotations = ([X_ax], sigy_2_5[end],text("2.5°", font25_c)))
# plot!(annotations = ([X_ax], sigy_5[end], text("5.0°", font5_c)))
# plot!(annotations = ([X_ax], sigy_7_5[end], text("7.5°", font75_c)))
# plot!(annotations = ([X_ax], sigy_10[end], text("10.0°", font10_c)))
