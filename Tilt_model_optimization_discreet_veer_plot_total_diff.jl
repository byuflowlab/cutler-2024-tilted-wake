...
function vel_deficit_plot_diff(tilt, X_D, def_z, C_T, sigy, sigz, Vel_slice, alpha_in, alpha_in_0, sigz0, sigy0)
    ...
    for i = 1:length(Y_values)
        ...
        for j = 2:length(Z_values)
            ...
            Vel_slice_val = Vel_slice[j,i]
            diff = abs((Vel_value*8.1)-Vel_slice_val)
            diff_0 = abs((Vel_value_0*8.1)-Vel_slice_val)
    ...
    # plot!(annotations = ([3.0], 0.75,text(L"\left|\Delta V_{org}\right|", fontylabel)))
    # savefig(wakesave2)

    # data1 = heatmap(Y_values, Z_values[2:end], Vel_diff_opt_grid, c=:Blues, clim=(0.0, 1.0))
    # plot(data1, xlimit = [-2.0, 2.0], colorbar_title="", right_margin=15Plots.mm, grid=:false, xlabel="y/D", ylabel="z*",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
    # plot!(annotations = ([3.0], 0.75,text(L"|\Delta V_{opt}|", fontylabel)))
    # savefig(wakesave1)
    ...
    # plot!(annotations = ([3.1], 0.75,text(L"\left|\Delta V_{org}\right| - |\Delta V_{opt}|", fontylabel)))
    # savefig(wakesave)
...
function objective_vel_wrapper_plot_difference_every_tilt(x, x0_opt, params)
    ...
    for i = 1:length(tilt_angles)
        ...
        wakesave = join(["Total_diff_0.pdf"])
        wakesave1 = join(["Total_diff_opt.pdf"])
        wakesave2 = join(["Total_diff_0_minus_opt.pdf"])

        fontylabel = text("", 15).font
        fontylabel.rotation=90
        # data1 = heatmap(Y_values, Z_values[2:end], vel0_total, c=:Blues)
        data1 = heatmap(Y_values, Z_values[2:end], vel0_total, c=:Blues, clim=(0.0, 20.0))
        plot(data1, colorbar_title="", xlimit = [-2.0, 2.0], right_margin=15Plots.mm, grid=:false, xlabel="y/D", ylabel="z*",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
        plot!(annotations = ([3.0], 0.75,text(L"\left|\Delta V\right|", fontylabel)))
        savefig(wakesave)

        # data1 = heatmap(Y_values, Z_values[2:end], velopt_total, c=:Blues)
        data1 = heatmap(Y_values, Z_values[2:end], velopt_total, c=:Blues, clim=(0.0, 20.0))
        plot(data1, colorbar_title="", xlimit = [-2.0, 2.0], right_margin=15Plots.mm, grid=:false, xlabel="y/D", ylabel="z*",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
        plot!(annotations = ([3.0], 0.75,text(L"\left|\Delta V\right|", fontylabel)))
        savefig(wakesave1)
        ...
        plot(data1, xlimit = [-2.0, 2.0], right_margin=15Plots.mm, grid=:false, xlabel="y/D", ylabel="z*",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
        plot!(annotations = ([3.1], 0.75,text(L"\left|\Delta V_{0}\right| - |\Delta V_{opt}|", fontylabel)))
        savefig(wakesave2)
...
function delta_V_pertilt_plot_diff_every_tilt(tilt, slices, Ct, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03, filename, alpha_in, alpha_in_0,
    ...
    wakesave = join([string(tilt*180/pi), "_0.pdf"])
    wakesave1 = join([string(tilt*180/pi), "_opt.pdf"])
    wakesave2 = join([string(tilt*180/pi), "_total.pdf"])

    fontylabel = text("", 15).font
    fontylabel.rotation=90
    # find vel_deficit at each of the points
    # data1 = heatmap(Y_values, Z_values[2:end], vel0, c=:Blues)
    data1 = heatmap(Y_values, Z_values[2:end], vel0, c=:Blues, clim=(0.0, 4.0))
    plot(data1, colorbar_title="", xlimit = [-2.0, 2.0], right_margin=15Plots.mm, grid=:false, xlabel="y/D", ylabel="z*",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
    plot!(annotations = ([3.0], 0.75,text(L"\left|\Delta V\right|", fontylabel)))
    savefig(wakesave)

    # data1 = heatmap(Y_values, Z_values[2:end], velopt, c=:Blues)
    data1 = heatmap(Y_values, Z_values[2:end], velopt, c=:Blues, clim=(0.0, 4.0))
    plot(data1, colorbar_title="", xlimit = [-2.0, 2.0], right_margin=15Plots.mm, grid=:false, xlabel="y/D", ylabel="z*",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
    plot!(annotations = ([3.0], 0.75,text(L"\left|\Delta V\right|", fontylabel)))
    savefig(wakesave1)

    # data1 = heatmap(Y_values, Z_values[2:end], vel_total_diff, c=:Blues)
    data1 = heatmap(Y_values, Z_values[2:end], vel_total_diff, c=:Blues, clim=(0.0, 0.3))
    plot(data1, xlimit = [-2.0, 2.0], right_margin=15Plots.mm, grid=:false, xlabel="y/D", ylabel="z*",yguidefontsize=fontsz,legendfontsize=fontsz,xguidefontsize=fontsz,xtickfontsize=fontsz,ytickfontsize=fontsz)
    plot!(annotations = ([3.1], 0.75,text(L"\left|\Delta V_{0}\right| - |\Delta V_{opt}|", fontylabel)))
    savefig(wakesave2)
...
function delta_V_pertilt_plot_diff_every_slice(j, tilt_angles, slice, Cts, kz1, kz2, kz3, ky1, ky2, sigy01, sigz01, sigz02, sigz03, filenames, alpha_in, alpha_in_0,
    ...
    wakesave = join([string(slice), "_0.pdf"])
    wakesave1 = join([string(slice), "_opt.pdf"])
    wakesave2 = join([string(slice), "_total.pdf"])
